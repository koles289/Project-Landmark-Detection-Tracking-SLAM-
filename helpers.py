...
def display_world(world_size, position, landmarks=None):
    ...
    sns.set_style("dark")
    # icons are downloaded for free from this website https://icons8.com/icons
    landmark= plt.imread('images/landmark.png')
    robot= plt.imread('images/robot_icon48.png')

    # Plot grid of values
    world_grid = np.zeros((world_size+1, world_size+1))

    # https://stackoverflow.com/questions/2318288/how-to-use-custom-png-image-marker-with-plot?noredirect=1&lq=1
    ax=plt.gca()
    cols = world_size+1
    rows = world_size+1
    ...
    im = OffsetImage(robot, zoom=72/ax.figure.dpi)
    im.image.axes = ax
    ab = AnnotationBbox(im, (position[0], position[1]), frameon=False, pad=0.0,)
    ax.add_artist(ab)
    ...
    if(landmarks is not None):
        ...
        for pos in landmarks:
            if(pos != position):
                # if I want to save resulution
                # im = OffsetImage(landmark, zoom=72/ax.figure.dpi)
                im = OffsetImage(landmark, zoom=.5)
                im.image.axes = ax
                ab = AnnotationBbox(im, (pos[0], pos[1]), frameon=False, pad=0.0,)
                ax.add_artist(ab)
...
# --------
# this routine makes the robot dat
